...
import {
  ...
  AtualizarStatusPagamentoOutput,
} from '@/infrastructure/dto/pagamento/atualizarStatusPagamento.dto';
...
export class PagamentoUseCase {
  ...
  async obterStatusPagamento(
    pedidoId: string,
  ): Promise<ObterStatusPagamentoOutput> {
  ...
  ): Promise<AtualizarStatusPagamentoOutput> {
    const pedido: Pedido = await this.pedidoRepository.findById(pedidoId);
